...
sdp_message_a_attribute_add (sdp_message_t * sdp, int pos_media, char *att_field, char *att_value)
{
  ...
  char *att_field_private;
  char *att_value_private;
  ...
  if (i != 0)
	...
	{
		//notify by hzc
		att_field_private = (char *)malloc(strlen(att_field)+1);
//		printf("strlen(att_field)=%d \r\n", strlen(att_field+1));
		memset(att_field_private , 0x00 , strlen(att_field)+1);
		att_value_private = (char *)malloc(strlen(att_value)+1);
//		printf("strlen(att_value)=%d \r\n", strlen(att_value)+1);
		memset(att_value_private , 0x00 , strlen(att_value)+1);
		strncpy(att_field_private , att_field, strlen(att_field));
		strncpy(att_value_private , att_value, strlen(att_value));
  		attr->a_att_field = att_field_private;
	    attr->a_att_value = att_value_private;
	}
